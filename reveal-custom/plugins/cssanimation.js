...
const RevealCSSAnimation = {
     ...
     */
		function parseIndices(str) {
			...
			for (let token of tokens) {
				if (token.includes("-")) {
					...
					} else indices.push(-from);
				} else indices.push(parseInt(token));
			}
		...
		function detectKeywords(tokens) {
			let preResult = [];
			for (let token of tokens) if (token.content || token.type === "string") preResult.push(token);

			for (let i = 0; i < preResult.length; ++i)
				if (
					preResult[i].type === "word" &&
					isKeyword(preResult[i].content) &&
					(i === 0 ||
						((preResult[i - 1].type !== "keyword" ||
							!["apply", "to", "execute", "show", "hide", "macro"].includes(preResult[i - 1].content)) &&
			...
			for (let i = preResult.length - 1; i >= 0; --i)
				if (
					preResult[i].type !== "string" &&
					preResult[i].content === ";" &&
					(i === preResult.length - 1 || preResult[i + 1].type === "keyword")
				)
					preResult[i].type = "keyword";
			...
			for (let i = 0; i < preResult.length; ++i)
				if (
					!(
						...
						i > 0 &&
						preResult[i - 1].type === "keyword" &&
						["initially", "next", "then"].includes(preResult[i - 1].content)
					)
				)
    ...
    */
		function tokenizeString(s, substitutions) {
			...
			for (let i = 0; i < result.length; ++i) {
				if (
					...
					result[i].content === "macro" &&
					(result[i + 1].type === "string" || result[i + 1].type === "word") &&
					result[i + 2].type !== "string" &&
					result[i + 2].content === "=" &&
			...
			if (!substitutions.length) return resultWithoutMacroCommands;
		...
		function getTime(tokenList) {
			...
			while (
				tokenList.length > 0 &&
				timeString.search(/^\d+([.,]\d+)?m?s/) === -1 &&
				["word", "string"].includes(tokenList[0].type)
			) {
     ...
     */
		...
		function getNextAnimationAtom(animationAtomScript) {
				...
				while (animationAtomScript.length > 0 && !["keyword", "indices"].includes(animationAtomScript[0].type)) {
					...
					if (animationAtomScript.length > 1 && animationAtomScript[1].content === "=") {
						...
						if (animationAtomScript.length > 0 && animationAtomScript[0].type === "string") {
							...
							while (
								animationAtomScript.length > 0 &&
								animationAtomScript[0].content !== "," &&
								["string", "word"].includes(animationAtomScript[0].type) &&
			...
			if (
				animationAtomScript.length > 1 &&
				animationAtomScript[0].type === "keyword" &&
				animationAtomScript[0].content === "rewind"
			) {
				animationAtomScript = animationAtomScript.slice(1);
				if (animationAtomScript.length > 1 && animationAtomScript[0].type === "string") {
					parsedAnimationAtom.rewindScript = parseAnimationRawScript(
						detectKeywords(tokenizeString(animationAtomScript[0].content)),
					);
					animationAtomScript = animationAtomScript.slice(1);
				}
			}
     ...
     */
		...
		function getAnimationObjectsForAtom(animationAtom, scope) {
			...
			for (let j = 0; j < animationObjectsUnfiltered.length; ++j) {
				if (
					!animationObjectsUnfiltered[j].classList.contains("custom-animation-carrier") &&
					animationObjectsUnfiltered[j].tagName.toLowerCase() !== "script"
				) {
			...
			for (let nStep = 0; nStep < animationSequence.length; ++nStep) {
				...
				for (let i = 0; i < animationStepAtoms.length; ++i) {
					...
					if (animationAtom.delay > 0 && !ignoreDelay) {
						setTimeout(function () {
							playAnimation([[animationAtom]], scope, true);
						}, animationAtom.delay);
						continue;
					}
					...
					for (let j = 0; j < animationObjects.length; ++j) {
						if (
							animationAtom.objectQueryIndices &&
							animationAtom.objectQueryIndices.indexOf(j) < 0 &&
							...
							for (let p in node.dataset) {
								if (p.startsWith("animationInitialBackup")) {
									...
									if (parameter.startsWith("class-")) {
										...
										if (node.dataset[p] === "true") node.classList.add(parameter);
										else node.classList.remove(parameter);
									} else if (parameter.startsWith("general-")) {
										parameter = parameter.slice("general-".length);
										node.setAttribute(parameter, node.dataset[p]);
									} else node.style[parameter] = node.dataset[p];
								} else if (p.search(/^dataAnimation\d+Backup/) === 0) node.removeAttribute(camelToDashed(p));
							}
						}
						if (animationType === "show") {
							animationType = "apply";
							animationAtom.propertiesToAssign = [];
							if (node.style.visibility !== "visible") animationAtom.propertiesToAssign.push(["visibility", "visible"]);
							if (node.style.opacity === "0") animationAtom.propertiesToAssign.push(["opacity", "1"]);
						}
						if (animationType === "hide") {
							animationType = "apply";
							animationAtom.propertiesToAssign = [];
							if (node.style.visibility !== "hidden") {
								if (animationAtom.duration > 0) animationAtom.propertiesToAssign.push(["opacity", "0"]);
								else animationAtom.propertiesToAssign.push(["visibility", "hidden"]);
							}
						}
						if (animationType === "apply") {
							...
							if (classesToToggle) {
								for (let k = 0; k < classesToToggle.length; ++k) {
									...
									if (node.classList.contains(classesToToggle[k])) node.classList.remove(classesToToggle[k]);
									else node.classList.add(classesToToggle[k]);
								}
							}
				...
				if (animationStepDuration > 0) {
					...
					setTimeout(() => playAnimation(remainingSteps, scope, false), animationStepDuration);
					return;
				}
			}
		}
				...
				for (let j = animationSequence[i].length - 1; j >= 0; --j) {
					...
					for (let k = 0; k < animationObjects.length; ++k) {
						...
						if (["apply", "show", "hide"].includes(animationAtom.animationType)) {
							let classes = [];
							if (animationAtom.classesToAdd) classes = classes.concat(animationAtom.classesToAdd);
							if (animationAtom.classesToRemove) classes = classes.concat(animationAtom.classesToRemove);
							if (animationAtom.classesToToggle) classes = classes.concat(animationAtom.classesToToggle);

							for (let m = 0; m < classes.length; ++m) {
								let cls = classes[m];
								if (node.getAttribute(backupAttrPrefix + "class-" + cls) === "false") node.classList.remove(cls);
								if (node.getAttribute(backupAttrPrefix + "class-" + cls) === "true") node.classList.add(cls);
							}
							...
							if (animationAtom.propertiesToAssign) {
								for (let m = 0; m < animationAtom.propertiesToAssign.length; ++m) {
									...
									} else if (node.hasAttribute(backupAttrPrefix + property))
