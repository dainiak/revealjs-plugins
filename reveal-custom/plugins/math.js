	...
	init: async (reveal) => {
		...
		options = {
			...
			svg: {
				...
				inheritAttributes: (options.svg && options.svg.inheritAttributes) || [
					"fill",
					"stroke",
					"fill-opacity",
					"id",
		...
		if (options.ignore.classes) {
			...
			options.ignore.classesRegExp = options.ignore.classesRegExp
				? "(" + options.ignore.classesRegExp + ")|" + regexp
				: regexp;
		}
		if (options.ignore.classesRegExp) window.MathJax.options.ignoreHtmlClass = options.ignore.classesRegExp;
		if (options.process.classesRegExp) window.MathJax.options.processHtmlClass = options.process.classesRegExp;
		...
		function typesetMathInSVG() {
			...
			function createBasicSvgMathNode(textNode) {
				...
				let svgMath = window.MathJax.tex2svg(mathMarkup, window.MathJax.getMetricsFor(textNode, isDisplay));
			...
			if (options.fragments.enabled && options.fragments.cssIndices) {
				...
				for (let slide of reveal.getSlides()) {
					...
					for (let fragment of fragmentsWithCssIndex) {
						...
						s = s.substring(s.indexOf(options.fragments.indexClassPrefix) + options.fragments.indexClassPrefix.length);
