...
let zoom = (function () {
	...
	function magnify(rect, scale) {
		...
		if (document.documentElement.classList)
			if (zoomedCurrently !== 1) document.documentElement.classList.add("zoomed");
...
const RevealZoom = {
	...
	init: (reveal) => {
		...
		let zoomPadding = options?.padding === 0 ? 0 : options?.padding || 20;
		...
		reveal.getViewportElement().addEventListener("mousedown", function (event) {
			if (!((event[zoomKey] || event[fullscreenKey]) && isEnabled)) return;
			...
			while (target) {
				...
				if (target.classList && target.classList.contains("MathJax")) break;

				if (
					(target.classList &&
						(target.classList.contains("smallest-zoomable") || target.classList.contains("zoomable"))) ||
					(target.hasAttribute && target.hasAttribute("data-smallest-zoomable"))
				)
					break;
			...
			if (
				!target ||
				(explicitZoomables &&
					!target.classList.contains("smallest-zoomable") &&
					!target.hasAttribute("data-smallest-zoomable") &&
			...
			if (event[fullscreenKey] && document.fullscreenEnabled) {
				target.addEventListener(
					"mousedown",
					function (evt) {
						if (document.fullscreenElement && evt[zoomKey] && evt[fullscreenKey]) document.exitFullscreen();
					},
					{ once: true },
				);
		...
		reveal.addEventListener("ready", function () {
			for (let e of document.querySelectorAll("img,video,span,div,section")) e.removeAttribute("title");
		});
	},
};
