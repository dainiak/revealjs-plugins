	...
	init: (reveal) => {
		...
		options = {
			urls: {
				...
				css:
					(options.urls && options.urls.css) || `https://cdn.jsdelivr.net/npm/katex@${katexVersion}/dist/katex.min.css`,
				autorender:
					(options.urls && options.urls.autorender) ||
		...
		for (let macroName in options.macros) {
			...
			if (macroDefinition instanceof Array) macroDefinition = macroDefinition[0];
		...
		for (let pair of options.delimiters.inline) delimiters.push({ left: pair[0], right: pair[1], display: false });

		let scriptsToLoad = [
			{
				url: options.urls.css,
				condition: !window.katex && !document.querySelector(`script[src="${options.urls.katex}"]`),
			},
			{
			...
			{
				url: options.urls.autorender,
				condition: !window.renderMathInElement && !document.querySelector(`script[src="${options.urls.autorender}"]`),
			},
		];
		...
		function typesetMathInSVG(renderOptions) {
			function getTargetProperties(node) {
				...
				while (t.tagName !== "svg") {
					...
					if (t.hasAttribute("dx")) properties.x += +fixPx(t.getAttribute("dx"));
					if (t.hasAttribute("dy")) properties.y += +fixPx(t.getAttribute("dy"));
					t = t.parentNode;
				}
				...
				if (
					options.svg.inheritAttributes.includes("classlist") ||
					options.svg.inheritAttributes.includes("classList")
				) {
					t = node;
			...
			function createSvgMathNode(textNode) {
				...
				foreignObjectNode.setAttribute("transform", "translate(" + x0 + " " + y0 + ")" + " scale(" + scale + ")");
			...
			window.addEventListener("load", function () {
				if (options.preamble && (typeof options.preamble === "string" || options.preamble === true)) {
					...
					try {
						...
						console.warn(
							"KaTeX error while loading the preamble: " +
								error.toString().replace("ParseError: KaTeX parse error: ", ""),
						);
					}
				}
