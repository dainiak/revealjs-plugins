...
const RevealInking = {
	...
	init: async (reveal) => {
		...
		options.ink = {
			colors: options.ink.color ||
				options.ink.colors || ["rgb(250,250,250)", "rgb(250,0,0)", "rgb(0,250,0)", "rgb(0,0,250)", "rgb(0,0,0)"],
		...
		let needToLoadOwnMath =
			options.math.enabled &&
			(reveal.getPlugin("math") || {}).renderer !== "mathjax" &&
			!(window.MathJax || {}).version;

		let scriptsToLoad = [
			{
				content:
					".ink-controls {position: fixed;bottom: 10px;right: 200px;cursor: default;" +
			...
			{
				url: "https://cdn.jsdelivr.net/npm/fabric@7.3.1/dist/index.min.js",
		...
		function toggleColorChoosers(b) {
			if (options.controls.colorChoosersAlwaysVisible && !b) return;

			for (let element of inkControlButtons.colorChoosers) element.style.visibility = b ? "visible" : "hidden";
		}

		function incrementLoadingQueue(s) {
			if (s) console.log("Loading resource: " + s);
		...
		function resetMainCanvasDomNode() {
			...
			if (controlsComputerStyle) canvasElement.style.zIndex = controlsComputerStyle.zIndex;
			else canvasElement.style.zIndex = "120";
		...
		function createSpotlight() {
			if (!options.spotlight.enabled) return;
			if (spotlight) destroySpotlight();
		...
		function destroySpotlight() {
			if (!(options.spotlight.enabled && spotlight)) return;
		...
		function toggleDrawingMode() {
			if (canvas.isDrawingMode) leaveDrawingMode();
			else {
				leaveDeletionMode();
				enterDrawingMode();
			}
		...
		function addMathImageEventListeners(img) {
			...
			img.on("selected", function () {
				if (canvas.getActiveObject() !== img) return;
		...
		function createNewFormulaWithQuery() {
			...
			let mathColor =
				(currentFormula && currentMathColor) || (options.math.color === "ink" ? currentInkColor : options.math.color);
			toggleControlButton(inkControlButtons.formula, true);
			inkControlButtons.formula.style.textShadow = "0 0 10px " + mathColor;
			...
			let svg = window.MathJax.tex2svg(options.math.preamble + formula, mjMetrics);
			...
			window.fabric
				...
				.then(function (result) {
					...
					for (let obj of objects) obj.set({ fill: mathColor });
					...
					if (targetScaleX) img.set({ scaleX: img.scaleX * targetScaleX, scaleY: img.scaleY * targetScaleY });

					if (targetAngle) img.set({ angle: targetAngle });
		...
		function loadCanvasFromMathEnrichedObject(serializedCanvas) {
			...
			return canvas.loadFromJSON(serializedCanvas).then(function () {
				...
				objects.forEach(function (obj) {
					...
					} else setCanvasObjectDefaults(obj);
				});
				canvas.requestRenderAll();
			});
		}
		...
		function addCanvasEventListeners() {
			canvas.on("mouse:down", function (opt) {
				...
				if (options.spotlight.enabled && opt.e.altKey) createSpotlight();
				else if (isMathImage(opt.target)) currentMathImage = opt.target;
			});
			canvas.on("mouse:up", function (opt) {
				isMouseLeftButtonDown = false;
				if (options.spotlight.enabled && opt.e.altKey) destroySpotlight();
			});
			...
			canvas.on("mouse:over", function (opt) {
				if (isInDeletionMode && isMouseLeftButtonDown) canvas.remove(opt.target);
			});

			canvas.on("object:added", function (opt) {
				if (!isMathImage(opt.target)) setCanvasObjectDefaults(opt.target);
			});
		...
		function documentKeyDownEventHandler(event) {
			...
			switch (event.key) {
				case options.hotkeys.toggleCanvas:
					toggleCanvas();
					break;
				case options.hotkeys.clear:
					if (isCanvasVisible()) canvas.clear();
					break;
				case options.hotkeys.serializeCanvas:
					if (isCanvasVisible()) serializeCanvasToFile();
					break;
				case options.hotkeys.draw:
					if (isCanvasVisible()) enterDrawingMode();
					break;
				case options.hotkeys.erase:
					if (isCanvasVisible()) {
						enterDeletionMode();
						canvas.selection = false;
					}
		...
		function documentKeyUpEventHandler(event) {
			if (options.math.enabled && event.key === options.hotkeys.insertMath && isCanvasVisible()) {
				createNewFormulaWithQuery();
			}
			...
			if (event.key === options.hotkeys.erase) leaveDeletionMode();
		...
		function restoreCanvasForCurrentSlide() {
			...
			loadCanvasFromMathEnrichedJSON(slide.dataset.inkingCanvasContent).then(() => {
				...
				if (slide.inkingObjectsPreload) {
					for (let obj of slide.inkingObjectsPreload) canvas.add(obj);
		...
		function addRevealEventListeners() {
			...
			reveal.addEventListener("overviewhidden", function () {
				let slide = reveal.getCurrentSlide();
				if (
					slide.hasAttribute("data-show-inking-canvas") ||
			...
			reveal
				...
				.forEach(function (slide, slideNumber) {
					if (slide.dataset.inkingCanvasContent) {
						...
						if (slide.id) slideContent.slideId = slide.id;
						else slideContent.slideNumber = slideNumber;
			...
			return window.fabric
				...
				.then(function (result) {
					...
					if (slide === reveal.getCurrentSlide()) {
						for (let obj of objects) canvas.add(obj);
						canvas.requestRenderAll();
					} else if (!slide.inkingObjectsPreload) slide.inkingObjectsPreload = objects;
					else for (let obj of objects) slide.inkingObjectsPreload.push(obj);
				})
				.catch(function (err) {
					decrementLoadingQueue();
		...
		function loadGlobalPredefinedCanvasContent() {
			function loadMultipleSlides(arrayOfContent, slides) {
				for (let c of arrayOfContent) {
					let slide;
					if (c.slideId) slide = document.getElementById(c.slideId);
					else if (c.slideNumber && slides && c.slideNumber < slides.length) slide = slides[c.slideNumber];
			...
			if (typeof options.inkingCanvasContent !== "string") {
				for (let slide of slides) slide.dataset.inkingCanvasContent = JSON.stringify(options.inkingCanvasContent);
				return;
			}
			...
			if (options.inkingCanvasContent.toLowerCase().endsWith(".json")) {
				...
				fetchText(url)
					.then((response) => {
						if (response.startsWith("{")) for (let slide of slides) slide.dataset.inkingCanvasContent = response;
						else loadMultipleSlides(JSON.parse(response), slides);
					})
					.catch((err) => {
						console.warn("Failed to fetch inking canvas content from " + url + ":", err);
					});
				return;
			}

			for (let slide of slides) slide.dataset.inkingCanvasContent = options.inkingCanvasContent;
		}

		function loadPredefinedCanvasContent() {
			if (options.inkingCanvasContent) loadGlobalPredefinedCanvasContent();
			...
			for (let slide of reveal.getSlidesElement().querySelectorAll("section[data-inking-canvas-src]")) {
				...
				if (!inkingCanvasSrc) continue;
				if (inkingCanvasSrc.toLowerCase().endsWith(".json")) {
					incrementLoadingQueue();
					fetchText(inkingCanvasSrc)
						.then((response) => {
						...
						.catch((err) => {
							decrementLoadingQueue();
							console.warn("Failed to fetch inking canvas JSON from " + inkingCanvasSrc + ":", err);
						});
					continue;
				}
			...
			if (wasCanvasVisible) toggleCanvas(true);
