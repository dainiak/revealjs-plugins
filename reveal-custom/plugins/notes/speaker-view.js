(function () {
    ...
    // Track every state string we've pushed into the preview iframe from the
    // main window within the last ~1.5s. The iframe emits its own slidechanged/
    // fragment events *after* each setState, and those echoes can arrive after
    // we've already moved on to a newer state. Without this, pressing space
    // rapidly in the main window makes the speaker view post stale echoes back
    // to main as setState, causing the deck to jump backwards.
    const appliedFromMain = new Set();
    ...
    window.addEventListener('message', function (event) {
        ...
        // Messages sent by the reveal.js inside of the current slide preview
            ...
            } else if (supportedEvents.includes(data.eventName)) {
                const incoming = JSON.stringify(data.state);
                // Drop echoes of anything we just pushed into the iframe from
                // the main window, not just the single most-recent state.
                if (!appliedFromMain.has(incoming) && currentState !== incoming) {
     ...
     */
    function dispatchStateToMainWindow(state) {
    ...
    /**
     * Applies an updated state from the main window to the speaker view.
     */
    function handleStateMessage(data) {

        // Store the most recently set state to avoid circular loops
        // applying the same state
        const stateKey = JSON.stringify(data.state);
        currentState = stateKey;
        appliedFromMain.add(stateKey);
        setTimeout(() => appliedFromMain.delete(stateKey), ECHO_GRACE_MS);
